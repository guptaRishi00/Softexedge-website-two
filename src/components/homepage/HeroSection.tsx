...
export default function HeroSection({ data }: any) {
  ...
  return (
    <div
      className="w-full lg:h-screen h-[600px] bg-cover bg-center top-0 left-0 rounded-3xl relative overflow-hidden"
      style={bacgroundImage}
    >
      <div className="absolute inset-0 bg-gray-800/30 backdrop-blur-sm flex flex-col justify-between gap-6 px-5 py-6 lg:p-10">
        <Header />

        <div className="flex flex-col items-center gap-10 text-center lg:gap-6">
          <p className="bg-white/10 text-white py-2 px-4 rounded-full">
            {data?.subtitle}
          </p>
          <h1 className="font-bold text-2xl lg:text-7xl flex flex-col items-center lg:gap-2 text-white">
          ...
          <div className="lg:flex lg:gap-6 flex-col lg:flex-row flex items-center gap-4 mt-4">
            <LinkComp
              ...
              color={letsTalkTheme}
              className="font-regular flex items-center lg:p-3 px-5 py-2 gap-3 lg:text-lg"
            >
              {data?.letsTalk?.text}
              {data?.letsTalk?.images.map((img: any, index: number) => (
                <Image
                  ...
                  width={36}
                  height={36}
                  alt="button logo lg:w-24 lg:h-24"
                />
              ))}
            </LinkComp>
            <LinkComp
