...
import { getStrapiMedia } from "@/lib/utils"; // Assuming you have this utility
...
export default function ServiceHighlight({ data }: ServiceHighlightProps) {
  if (!data) return null;

  // Handle Image URL (safeguard against missing data)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
        ...
        <div className="flex flex-col gap-4 justify-center">
          {data.lists?.map((item, index) => (
            <div
              key={item.id || index}
              className="bg-[#111111] border border-white/5 rounded-3xl px-6 py-5 md:py-6 md:px-8 flex items-center gap-5 transition-colors hover:bg-[#161616] group"
            >
              {/* Blue Dot Indicator */}
              <div className="shrink-0 w-3 h-3 md:w-3.5 md:h-3.5 rounded-xl bg-gradient-to-r from-[#3445E7] via-[#2F85EA] to-[#07D6F3] shadow-[0_0_10px_rgba(47,133,234,0.5)]" />
